...
export function activate(context: vscode.ExtensionContext) {
	...
	*/
	...
	let saveListener = vscode.workspace.onDidSaveTextDocument((savedDoc: vscode.TextDocument) =>
		doctestLinter(savedDoc)
	);
...
function doctestHandler(activeEditor: vscode.TextEditor | undefined, docChange?: vscode.TextDocumentChangeEvent): void {
	...
	*/
	if (
		(docChange && docChange?.document.fileName !== activeEditor?.document.fileName) ||
	...
	if (docData?.totalDoctests > 0) {
		// If there are doctests, show button and status bar items.
...
function doctestExecuter() {
	...
	*/
	...
	if (findTerminal("Python") > -1) {
		...
		// Next check for "Doctest" terminal
