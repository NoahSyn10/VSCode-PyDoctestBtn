	...
	doctestLinter(doctestOutput: string, callback: Utils["failureCallback"]) {
        ...
        */
		if (doctestOutput === "") {
			callback();
			return;
		} // Callback with no value if no output.
		...
		let promises = outputBlocks.map((failure) => {
			// Use promises to return result after object creation.
		...
		Promise.all(promises).then((failureList) => {
			callback(failureList);
		}); // Resolve all promises then run callback with mapped list.
	}
}
